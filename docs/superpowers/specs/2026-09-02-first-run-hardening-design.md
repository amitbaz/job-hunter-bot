...
### 1. Resilient targeted search

Introduce a small search-backend abstraction used by market targeted search and canonical lookup.

- If `BRAVE_SEARCH_API_KEY` is configured, use Brave Search API first.
- If it is absent, retain DuckDuckGo HTML as the zero-key backend.
- If Brave fails for a query, fall back to DuckDuckGo for that query.
...
- Canonical targeted lookup uses the same backend chain rather than constructing a separate DuckDuckGo-only source.

Brave is optional because it requires account/API-key setup. The no-key behavior remains supported.
...
- Return a `CandidateContextResult` carrying `context` plus `source` (`cache`, `gemini`, `fallback_empty_profile`, or `fallback_error`).
...
- Cap a single wait to the rolling-window duration plus a small safety margin; if capacity still cannot be obtained after the retry, propagate the temporary exception and let existing queue behavior defer work.
...
- Rename pre-delivery digest candidate counts internally; `delivered` in the market log must count only jobs for which a Telegram message/card delivery was successfully persisted during this run.
...
4. Search statistics recorded after discovery, including zero-result success.
5. Canonical lookup uses the shared search backend.
6. Candidate-context parse/API error logs fallback source without leaking profile/response.
7. Candidate context is requested once per pipeline run.
8. RPM-only internal pressure returns a retry delay; RPD exhaustion remains hard budget exhaustion.
9. Gemini client waits/retries without issuing a provider call before capacity.
10. Market `delivered` counts only successful Telegram delivery.
11. Reattribution metric explains market count shifts.
