...
def _build_evaluation_prompt(job: Job, profile: str, policy: SearchPolicy) -> str:
...
Rules:
- Only use evidence present in the candidate profile and job description below. Never invent candidate facts.
...
def evaluate_job(job: Job, profile: str, policy: SearchPolicy, gemini: "GeminiClient") -> Evaluation:
    raw = gemini.generate_text(_build_evaluation_prompt(job, profile, policy), json_mode=True)
